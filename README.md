<div align="center">

  
# Marius Nikolai Dale
 
**M.Sc. Information Technology & Economics · University of Bergen**
 
[![LinkedIn](https://img.shields.io/badge/LinkedIn-marius--dale-0A66C2?style=flat&logo=linkedin)](https://linkedin.com/in/marius-dale)
[![GitHub followers](https://img.shields.io/github/followers/mariusdale?style=flat&label=Follow&color=00e5c0&labelColor=0a0f1a)](https://github.com/mariusdale)
 
</div>

 
## About
 
Master's student working at the intersection of **AI agents, fintech infrastructure, and developer tooling**. Currently building governance primitives for AI agent payments and shipping CLI tooling for full-stack scaffolding.
 
- 🎓 M.Sc. IT & Economics @ **University of Bergen** (2023–2028)
- 🏦 Incoming Summer Intern @ **Storebrand Sandbox** — fintech program
- 📈 Portfolio group @ **Fintech Enigma** — Student-run investment fund
---
 
## Featured Projects


### ⚙️ [Assembler](https://github.com/mariusdale/Assembler)
> CLI-tool that deploys production-ready Next.js applications with **Clerk** authentication and **Neon** Postgres provisioning out of the box. Built in TypeScript with macOS Keychain-backed credential storage via `keytar`.
 
### 🧾 AgentLedger
> Governance and budget control layer for AI agent payments — *Ramp for AI agents*. Compliance-grade agent governance for regulated fintech, with a Know-Your-Agent (KYA) identity layer and integrations across emerging agent payment protocols (x402, AP2, ACP). Turborepo monorepo (TypeScript), Next.js dashboard, MCP server, and AgentKit extension.
 
### 🔍 [adf-inspect](https://github.com/mariusdale/adf-inspect)
> Open-source CLI for inspecting and visualizing **Azure Data Factory** pipelines — list pipelines, view dependency trees, audit parameters, and run static analysis from the terminal.
 
---
 
## Tech Stack
 
**Languages**
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
 
**Frameworks & Runtime**
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=next.js&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
 
**AI**
![Claude](https://img.shields.io/badge/Claude-D97706?style=flat&logo=anthropic&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white)
![MCP](https://img.shields.io/badge/MCP-000000?style=flat)
 
**Infra & Data**
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat&logo=microsoftazure&logoColor=white)
![Neon](https://img.shields.io/badge/Neon-00E599?style=flat&logo=neon&logoColor=black)
![Postgres](https://img.shields.io/badge/Postgres-336791?style=flat&logo=postgresql&logoColor=white)
 
---
 
<sub>Bergen, Norway</sub>
 
</div>
